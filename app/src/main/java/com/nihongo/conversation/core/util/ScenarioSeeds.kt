...
/**
 * Centralized scenario seed data
 * Extracted from DataInitializer for maintainability
 *
 * To add a new scenario:
 * 1. Add Scenario object to the scenarios list below
 * 2. Set unique slug and promptVersion = 1
 * 3. Increment promptVersion when updating content
 */
@Singleton
class ScenarioSeeds @Inject constructor() {

    companion object {
        private const val TAG = "ScenarioSeeds"
...
$FORMAT_RULES
    ...
    /**
     * Seed all scenarios into database using upsert logic
     */
    suspend fun seedAll(scenarioDao: ScenarioDao) {
        android.util.Log.d(TAG, "🌱 Seeding ${scenarios.size} scenarios...")
        scenarios.forEach { scenario ->
            scenarioDao.upsertBySlug(scenario)
        }
        android.util.Log.d(TAG, "✅ Scenario seeding complete")
    }
}
